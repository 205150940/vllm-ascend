...
def kill_process(exclude_dp_ranks):
    for dp_rank in exclude_dp_ranks:
        match_key = f"VLLM::EngineCore_DP{dp_rank}"
        cmd = f"ps -ef | grep '{match_key}'"
        result = subprocess.run(
            cmd, shell=True, capture_output=True, text=True
        )
        print(result)
        process_info = result.stdout.strip()
        pid = process_info.split()[1] if process_info else ""
        subprocess.run(
            ["kill", "-9", str(pid)],
            check=True,
            capture_output=True,
...
def main():
    ...
    success_pause = kill_process(args.exclude_dp_ranks)
